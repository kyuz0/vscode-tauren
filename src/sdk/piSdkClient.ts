...
export class PiSdkClient implements PiClient {
  ...
  public async updateRuntimeSetting(settingId: PiSettingId, value: SettingValue): Promise<{ applied: 'live' | 'reload'; message?: string }> {
    ...
    switch (settingId) {
      ...
      case 'quietStartup':
        this.getSettingsManager().setQuietStartup(this.requireBoolean(value, settingId));
        await this.flushSettings();
        return { applied: 'live', message: 'Quiet startup updated.' };
  ...
  private getRuntimeSettingsState(): Partial<PiSessionState> {
    ...
    return {
      ...
      quietStartup: callOptionalSettingGetter<boolean>(settingsManager, 'getQuietStartup'),
